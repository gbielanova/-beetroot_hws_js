//task1
//add support for Command button, so MAC users can play as well
const divEl = document.getElementById('divToEdit');
const textAreaEl = document.getElementById('textAreaToEdit');

window.addEventListener('keydown', function (event) {
    if ((event.metaKey == true || event.ctrlKey == true) && event.code == "KeyE") {
...
window.addEventListener('keydown', function (event) {
    if ((event.metaKey == true || event.ctrlKey == true) && event.key == '+') {
        event.preventDefault();
